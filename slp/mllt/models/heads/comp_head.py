...
class CompHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_mean = torch.nn.Parameter(torch.zeros([self.num_classes]),requires_grad=True)
        self.pos_std = torch.nn.Parameter(torch.ones([self.num_classes]),requires_grad=True)
        self.neg_mean = torch.nn.Parameter(torch.zeros([self.num_classes]),requires_grad=True)
        self.neg_std = torch.nn.Parameter(torch.ones([self.num_classes]),requires_grad=True)
    ...
    def loss(self,
        ...
        #std compensation
        cls_score  = cls_score*(self.pos_std*labels + self.neg_std*(1-labels))
        #mean compensation
        cls_score  = cls_score + self.pos_mean*labels + self.neg_mean*(1-labels) 
